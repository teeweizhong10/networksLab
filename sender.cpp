...
/*
 * in main() of sender, a for loop will iterate through the packets vector.
 * in the for loop there will be if elses to decide which method to run.
 * stop and wait: stopandwaitSender(i)
 * GBN: GBNSender(window size)-would need to be out of for loop
 * selective repeat: selectiverepeatSender()
 */
...
//SENDER LOGIC (stop and wait)
void stopandwaitSender(int waitTime, int currrentPacket){
    bool ackReceived=false;
    int sentPackets=currentPacket;
       //check if current packet needs to be dropped
       if(packets[sentPackets].getPacketNum()==packetsToDrop[0]){//this assumes packetsToDrop<> is ordered
           packetsToDrop.erase(packetsToDrop.begin());//remove first element of packetsToDrop<>
           //"send packet"
           cout<< "Packet "<< packets[sentPackets].getPacketNum() << "sent";
           //sleep_for(ms(waitTime)); //LOCAL?
           usleep(waitTime*1000)
           cout<< "Packet "<< packets[sentPackets].getPacketNum() << "******** TIMED OUT ********";
           //actually send packet:
           stopandwaitSender(waitTime,sentPackets); //recursive call

       }else{//send packet normally
           std::clock_t start;

           start = std::clock();//timer started
           //send packets[sentPackets].getPacketMessage()
           cout<< "Packet "<< packets[sentPackets].getPacketNum() << "sent";

           /*
            * TODO: message from receiver: update ackRecieved, stop Timer
            */

           //below wouldnt work

           //check for timeout:
           if(ackReceived==true){//if received the ack (SENT SUCCESSFULLY)
               cout<< "Packet "<< packets[sentPackets].getPacketNum() << "sent";
           }else if(((std::clock() - start) / (double)(CLOCKS_PER_SEC / 1000)>waitTime)) & ackReceived==false){ //if current time in ms > wait time & havent received ack...
            cout<< "Packet "<< packets[sentPackets].getPacketNum() << "******** TIMED OUT ********";
